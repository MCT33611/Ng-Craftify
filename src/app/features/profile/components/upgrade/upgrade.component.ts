...
export class UpgradeComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.filteredSkills = this.subscriptionForm
      .get('skillCtrl')
      ?.valueChanges.pipe(
        startWith(null),
        map((skill: string | null) =>
          skill ? this.skills_filter(skill) : this.allSkills.slice()
        )
      );
  }
  ...
  onSuccess = async (getSubscription: {
    paymentId: string;
    userId: string;
    planId: string;
  }) => {
  ...
  add(event: MatChipInputEvent): void {
    ...
    if (value) {
      if (value.includes(',')) this.skills.push(...value.split(','));
      else this.skills.push(value);
    }
